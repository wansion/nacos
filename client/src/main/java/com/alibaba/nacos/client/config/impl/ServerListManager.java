...
public class ServerListManager {
  ...
  public ServerListManager(List<String> fixed, String namespace) {
    ...
    if (StringUtils.isBlank(namespace)) {
      name =
          FIXED_NAME
              + "-"
              + getFixedNameSuffix(serverAddrs.toArray(new String[serverAddrs.size()]));
    } else {
  ...
  public ServerListManager(String endpoint, String namespace) throws NacosException {
    ...
    if (StringUtils.isBlank(namespace)) {
      name = endpoint;
      addressServerUrl =
          String.format("http://%s:%d/%s/%s", endpoint, endpointPort, contentPath, serverListName);
    } else {
      if (StringUtils.isBlank(endpoint)) {
        throw new NacosException(NacosException.CLIENT_INVALID_PARAM, "endpoint is blank");
      }
      ...
      addressServerUrl =
          String.format(
              "http://%s:%d/%s/%s?namespace=%s",
              endpoint, endpointPort, contentPath, serverListName, namespace);
    }
  }
  ...
  public ServerListManager(Properties properties) throws NacosException {
    ...
    if (StringUtils.isNotEmpty(serverAddrsStr)) {
      ...
      if (StringUtils.isBlank(namespace)) {
        ...
        addressServerUrl =
            String.format(
                "http://%s:%d/%s/%s", endpoint, endpointPort, contentPath, serverListName);
      } else {
        this.namespace = namespace;
        this.tenant = namespace;
        name = endpoint + "-" + namespace;
  ...
  private String initEndpoint(final Properties properties) {
    ...
    String endpointPortTmp =
        TemplateUtils.stringEmptyAndThenExecute(
            System.getenv(PropertyKeyConst.SystemEnv.ALIBABA_ALIWARE_ENDPOINT_PORT),
            new Callable<String>() {
              @Override
              public String call() {
    ...
    String isUseEndpointRuleParsing =
        properties.getProperty(
            ...
            System.getProperty(
                SystemPropertyKeyConst.IS_USE_ENDPOINT_PARSING_RULE,
  ...
  public synchronized void start() throws NacosException {
    ...
    if (serverUrls.isEmpty()) {
      LOGGER.error(
          "[init-serverlist] fail to get NACOS-server serverlist! env: {}, url: {}",
          name,
          addressServerUrl);
      throw new NacosException(
          NacosException.SERVER_ERROR,
          "fail to get NACOS-server serverlist! env:"
              + name
              + ", not connnect url:"
              + addressServerUrl);
    }
  ...
  class GetServerListTask implements Runnable {
    ...
    public void run() {
      ...
      try {
        ...
        LOGGER.error(
            "[" + name + "][update-serverlist] failed to update serverlist from address server!",
            e);
      }
    }
  }
  ...
  private List<String> getApacheServerList(String url, String name) {
    try {
      ...
      if (HttpURLConnection.HTTP_OK == httpResult.code) {
        ...
        LOGGER.error(
            "[check-serverlist] error. addressServerUrl: {}, code: {}",
            addressServerUrl,
            httpResult.code);
        return null;
      }
